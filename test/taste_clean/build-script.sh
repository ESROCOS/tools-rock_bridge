...
cd "$SKELS" && rm -f camera_rock_bridge.zip && zip camera_rock_bridge camera_rock_bridge/* && cd $OLDPWD

cd "$SKELS" && rm -f pus.zip && zip pus pus/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subCPP camera_rock_bridge:"$SKELS"/camera_rock_bridge.zip \
	--subC pus:"$SKELS"/pus.zip \
